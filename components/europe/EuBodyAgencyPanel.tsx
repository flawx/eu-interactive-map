...
function categoryLabel(
  category: EuBodyAgencyCategory,
  t: ReturnType<typeof getMessages>["euBodyAgencyPanel"],
): string {
  switch (category) {
    case "eu-body":
      return t.categoryEuBody;
    case "decentralised-agency":
      return t.categoryDecentralisedAgency;
    case "cfsp-agency":
      return t.categoryCfspAgency;
    case "other":
      return t.categoryOther;
  }
}

export default function EuBodyAgencyPanel({
  agencyId,
  locale,
  onClose,
  onRouteToPlace,
}: EuBodyAgencyPanelProps) {
  const t = getMessages(locale);
  const tp = t.euBodyAgencyPanel;
  const agency = getEuBodyAgencyById(agencyId) ?? null;
  ...
  const countryLabel =
    regionNames?.of(agency.countryCode === "EL" ? "GR" : agency.countryCode) ??
    agency.countryCode;
  const source = DATA_SOURCES_REGISTRY.find((entry) =>
    agency.sourceIds.includes(entry.id),
  );
    ...
    >
      <header className="sticky top-0 z-[5] shrink-0 border-b border-[var(--map-ui-border)] bg-[var(--map-ui-surface)] px-4 py-3 backdrop-blur-md">
        <div className="flex items-start gap-3">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-[#facc15]/30 bg-[#6d28d9] text-[#facc15] shadow-sm">
        ...
        <p className="mt-2 inline-flex rounded-full border border-[#6d28d9]/40 bg-[#6d28d9]/20 px-2 py-0.5 text-[10px] font-medium text-violet-200">
          {tp.badge}
        </p>
        {onRouteToPlace ? (
          <div className="mt-2">
      ...
      <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-4 py-3">
        <section className="mb-4">
          ...
          <p className="text-sm leading-relaxed text-[var(--map-ui-text)]">
            {categoryLabel(agency.officialCategory, tp)}
          </p>
        </section>
        ...
        <section className="mb-4">
          ...
          >
            {agency.officialUrl.replace(/^https?:\/\//, "").replace(/\/$/, "")}
